...
- **Lernkarten:** eigener Fragenpool pro Modul mit Leitner-Spaced-Repetition (1/3/7/14/30 Tage), „heute fällig“-Widget, **MC-Quiz-Modus** mit Einzel- **und Mehrfachauswahl** (`correct`-Array → „Alle zutreffenden auswählen“) und **Import/Export** (JSON oder CSV `frage;antwort;falsch1;falsch2…`, duplikatsicher) — fertige Prompt-Vorlage zum Generieren aus Skripten: [PROMPT.md](PROMPT.md). Bei verbundenem Studienordner direkt als `<MODUL>/lernkarten.json` sichern/laden.
